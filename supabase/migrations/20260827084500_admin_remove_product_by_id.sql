-- Retirada administrativa exige papel admin e AAL2. A função do vendedor
-- permanece separada para não elevar permissões do fluxo de autogestão.
...
BEGIN
  ...
  RETURN jsonb_build_object('status', action_status, 'product_id', _product_id, 'name', target_product.name, 'seller_id', target_product.seller_id);
END;
$$;

REVOKE ALL ON FUNCTION public.admin_remove_product(bigint, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_remove_product(bigint, text) TO authenticated, service_role;
