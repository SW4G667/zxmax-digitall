  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "register") {
        ...
        if (passStrength < 2) { setError("Senha muito fraca. Use letras maiúsculas, números e símbolos."); setLoading(false); return; }
        ...
        if (err) {
          ...
          else if (err.includes("Email not confirmed")) setError("Confirme seu e-mail antes de fazer login.");
          else setError(err);
        }
      }
    } catch {
      ...
      <div className="w-full max-w-md relative z-10 bg-[#0a0a0f] border border-white/10 rounded-2xl p-8 shadow-2xl animate-fade-in-up">
        ...
        {needsMfa ? (
          <div className="text-center">
            ...
            <h1 className="text-xl font-black text-white">Verificação em duas etapas</h1>
            <p className="text-xs text-white/50 mt-2 mb-2">Código do seu app autenticador</p>
            <div className="inline-flex items-center gap-1.5 text-[11px] bg-primary/10 text-primary border border-primary/20 px-3 py-1 rounded-full mb-6">
              <ShieldCheck className="w-3 h-3" /> Protegido com 2FA
            </div>
            ...
            <div className="flex justify-center gap-2 mb-5" onPaste={handleCodePaste}>
              {code.map((d, i) => (
                <input
                  ...
                  className="w-12 h-14 text-center text-xl font-black rounded-xl bg-white/[0.04] border border-white/10 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none text-white"
                />
              ))}
            </div>

            {error && <div className="mb-4 p-3 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-xs flex items-center gap-2"><AlertTriangle className="w-4 h-4" /> {error}</div>}

            <button
              onClick={() => handleVerifyMfa()}
              disabled={verifying || code.join("").length !== 6}
              className="w-full bg-primary text-white py-3.5 rounded-xl text-sm font-black flex items-center justify-center gap-2 disabled:opacity-50 shadow-lg shadow-primary/20 hover:bg-primary/90 transition"
            >
              {verifying ? <Loader2 className="w-5 h-5 animate-spin" /> : <ShieldCheck className="w-5 h-5" />}
              {verifying ? "Verificando..." : "Confirmar código"}
            </button>

            <p className="text-[11px] text-white/30 mt-4">Abra seu Google Authenticator e digite o código de 6 dígitos. Ele muda a cada 30s.</p>
          </div>
        ) : (
          <>
            <div className="text-center mb-6">
              ...
              <div className="flex items-center justify-center gap-2 mt-2 text-[11px] text-white/40">
                <Shield className="w-3 h-3 text-success" /> Compra Protegida <Zap className="w-3 h-3 text-yellow-500" /> Entrega Imediata
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              {mode === "register" && (
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome Completo" className="w-full p-3.5 rounded-xl bg-white/[0.04] border border-white/10 focus:border-primary/50 focus:ring-2 focus:ring-primary/20 outline-none text-white placeholder:text-white/30 text-sm" />
              )}
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="E-mail" required autoComplete="email" className="w-full p-3.5 rounded-xl bg-white/[0.04] border border-white/10 focus:border-primary/50 focus:ring-2 focus:ring-primary/20 outline-none text-white placeholder:text-white/30 text-sm" />
              <div className="relative">
                <input type={showPass ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Senha" required autoComplete={mode === "login" ? "current-password" : "new-password"} className="w-full p-3.5 pr-10 rounded-xl bg-white/[0.04] border border-white/10 focus:border-primary/50 focus:ring-2 focus:ring-primary/20 outline-none text-white placeholder:text-white/30 text-sm" />
                <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/30 hover:text-white"><Eye className={`w-4 h-4 ${showPass ? "hidden" : ""}`} /><EyeOff className={`w-4 h-4 ${showPass ? "" : "hidden"}`} /></button>
              </div>
              {mode === "register" && password && (
                <div className="flex gap-1">
                  {[...Array(4)].map((_, i) => (
                    <div key={i} className={`h-1 flex-1 rounded-full transition ${i < passStrength ? (passStrength <= 2 ? "bg-destructive" : passStrength === 3 ? "bg-yellow-500" : "bg-success") : "bg-white/10"}`} />
                  ))}
                </div>
              )}
              {mode === "register" && (
                <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Confirmar Senha" className="w-full p-3.5 rounded-xl bg-white/[0.04] border border-white/10 focus:border-primary/50 outline-none text-white placeholder:text-white/30 text-sm" />
              )}
              {error && <div className="p-3 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-xs flex items-center gap-2"><AlertTriangle className="w-4 h-4" /> {error}</div>}
              <button type="submit" disabled={loading} className="w-full bg-primary text-white py-3.5 rounded-xl text-sm font-black flex items-center justify-center gap-2 disabled:opacity-50 shadow-lg shadow-primary/20 hover:bg-primary/90 transition">
                <Lock className="w-4 h-4" />
                {loading ? "Aguarde..." : mode === "login" ? "Acessar Plataforma" : "Criar Conta"}
              </button>
            </form>

            {mode === "login" && <button type="button" onClick={handleForgot} className="w-full text-center text-xs font-bold text-primary mt-3 hover:underline">Esqueceu sua senha?</button>}
            ...
            <div className="mt-5">
              ...
              <p className="text-[10px] text-white/20 text-center mt-3 flex items-center justify-center gap-1"><ShieldCheck className="w-3 h-3" /> Protegido com 2FA • Criptografia de ponta</p>
            </div>
          </>
        )}
      </div>
